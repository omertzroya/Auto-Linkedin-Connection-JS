...
Linkedin = {
  ...
  clickDone: function (data, config) {
    // Selecting the 'Send now' button by its aria-label and class attributes for greater accuracy
    var sendButtons = document.querySelectorAll("button.artdeco-button--primary[aria-label='Send now']");
    // Check if the button is found using the primary method
    if (sendButtons.length > 0) {
      console.debug("DEBUG: clicking 'Send now' button to close popup");
      sendButtons[0].click();
    } else {
      // Fallback method: Try finding the button based on the inner text 'Send without a note'
      var buttons = document.querySelectorAll("button.artdeco-button--primary");
      var sendButton = Array.from(buttons).find(button => button.innerText.includes("Send without a note"));
      if (sendButton) {
        console.debug("DEBUG: clicking 'Send without a note' button to close popup");
        sendButton.click();
      } else {
        console.debug("DEBUG: 'Send now' button not found, attempting to close the popup by other means");
        // Implement any additional logic to handle the case where the button is not found
      }
    }
